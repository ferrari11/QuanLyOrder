...
export default function SettingsView({
  ...
  // Email Auth states
  const [email, setEmail] = useState('lekhiemlv@gmail.com');
  ...
  const handleGoogleConnectClick = async () => {
    try {
      ...
      console.warn("Google OAuth failed or cancelled, showing email login option instead:", err);
      setShowManualAuth(true);
      setAuthError('Không thể mở popup kết nối Google. Vui lòng nhập Email & Mật khẩu bên dưới để kết nối.');
    }
  };
            ...
            {!googleToken ? (
              <div className="space-y-3.5">
                {!showManualAuth ? (
                  ...
                  <form onSubmit={handleAuthSubmit} className="space-y-3 bg-gray-50 border border-gray-150 p-3.5 rounded-xl animate-fade-in">
                    <div className="flex items-center justify-between border-b border-gray-150 pb-1.5">
                      <span className="text-[10px] font-bold text-gray-700 uppercase tracking-wider">Đăng nhập tài khoản Google</span>
                      <button 
                        type="button" 
                        onClick={() => {
                    ...
                    {authError && (
                      <div className="p-2 bg-red-50 text-red-700 border border-red-100 rounded-lg text-[10px] font-semibold">
                        {authError}
                      </div>
                    )}
                    
                    <div className="space-y-1">
                      <label className="text-[9px] font-bold text-gray-400 uppercase tracking-wider block">Email</label>
                      <input
                        type="email"
                        required
                        ...
                        className="w-full px-2.5 py-1.5 border border-gray-200 rounded-lg text-xs focus:outline-none focus:border-primary placeholder:text-gray-300"
                      />
                    </div>
                    
                    <div className="space-y-1">
                      <label className="text-[9px] font-bold text-gray-400 uppercase tracking-wider block">Mật khẩu</label>
                      <input
                        type="password"
                        required
                        placeholder="••••••"
                    ...
                    <button
                      ...
                      className="w-full py-2 bg-primary hover:bg-primary/95 text-white text-xs font-bold rounded-lg shadow-xs transition-colors cursor-pointer active-press disabled:opacity-50 flex items-center justify-center gap-1"
                    >
                      {authLoading ? (
                        <>
